-- Obsidian UI Fork - fixed header (works on all executors)
...
local getgenv = rawget(_G, "getgenv") or function()
    return shared or _G
end
...
local delfile = rawget(_G, "delfile")
...
local gethui = rawget(_G, "gethui") or function()
    return CoreGui
end
...
function Library:GetIcon(IconName)
    ...
    local Ok, Icon = pcall(function()
        if IconsModule.GetAsset then return IconsModule.GetAsset(IconName) end
        return nil
    end)
...
function Library:CreateWindow(WindowInfo)
    ...
    -- Mobile: use smaller window so it fits the screen
    if Library.IsMobile then
        local mx = math.floor(ViewportSize.X * 0.92)
        local my = math.floor(ViewportSize.Y * 0.75)
        WindowInfo.Size = UDim2.fromOffset(math.clamp(mx, 300, 700), math.clamp(my, 220, 500))
    end

    local MaxX = math.max(ViewportSize.X - 32, 280)
    local MaxY = math.max(ViewportSize.Y - 32, 200)
    ...
    WindowInfo.Size = UDim2.fromOffset(
        math.clamp(WindowInfo.Size.X.Offset, math.min(Library.MinSize.X, MaxX), MaxX),
        math.clamp(WindowInfo.Size.Y.Offset, math.min(Library.MinSize.Y, MaxY), MaxY)
    )
    ...
    if listfiles then
        ...
        if Ok and type(Files) == "table" then
            for _, File in pairs(Files) do
                ...
                if Name then Library._ConfigCache[Name] = true end
            end
        end
    end
...
function Library:CreateConfig(Name)
    ...
    if Library._ConfigCache[Name] or (isfile and isfile(Path)) then
        Library:Notify({ Title = "Config", Description = "Already exists", Time = 3 })
        return false
    end
    local Ok, Enc = pcall(function()
        return HttpService:JSONEncode({ Toggles = {}, Options = {}, Keybinds = {} })
    end)
    if Ok and pcall(writefile, Path, Enc) then
        Library._ConfigCache[Name] = true
        Library:Notify({ Title = "Config Created", Description = "Created \"" .. Name .. "\"", Time = 3 })
        return true
    end
    Library:Notify({ Title = "Config", Description = "Failed to create", Time = 3 })
    return false
end

function Library:SaveConfig(Name)
    Name = tostring(Name or "")
    if not writefile or Name == "" then
        Library:Notify({ Title = "Config", Description = "Cannot save", Time = 3 })
        return false
    end
    EnsureFolder(Library.ConfigFolder)
    ...
    if Ok and pcall(writefile, Library.ConfigFolder .. "/" .. Name .. ".json", Enc) then
        Library._ConfigCache[Name] = true
        Library:Notify({ Title = "Config Saved", Description = "Saved \"" .. Name .. "\"", Time = 3 })
        return true
    end
...
function Library:LoadConfig(Name)
    Name = tostring(Name or "")
    if not readfile or not isfile or Name == "" then return false end
    local Path = Library.ConfigFolder .. "/" .. Name .. ".json"
    if not isfile(Path) then
        Library:Notify({ Title = "Config", Description = "Not found", Time = 3 })
        return false
    end
    ...
    Library:Notify({ Title = "Config Loaded", Description = "Loaded \"" .. Name .. "\"", Time = 3 })
    return true
end
...
function Library:DeleteConfig(Name)
    ...
    local Path = Library.ConfigFolder .. "/" .. Name .. ".json"
    if delfile and isfile and isfile(Path) then pcall(delfile, Path) end
...
function Library:GetThemes()
    ...
    if listfiles then
        local Ok, Files = pcall(listfiles, Library.ThemeFolder)
        if Ok and type(Files) == "table" then
            for _, File in pairs(Files) do
                local Name = tostring(File):match("([^/\\]+)%.json$")
                if Name then Library._ThemeCache[Name] = true end
            end
        end
    end
...
function Library:CreateTheme(Name)
    Name = tostring(Name or ""):gsub("^%s+", ""):gsub("%s+$", "")
    if not writefile or Name == "" then
        Library:Notify({ Title = "Theme", Description = "Cannot create", Time = 3 })
        return false
    end
    EnsureFolder(Library.ThemeFolder)
    local Path = Library.ThemeFolder .. "/" .. Name .. ".json"
    if Library._ThemeCache[Name] or (isfile and isfile(Path)) then
        Library:Notify({ Title = "Theme", Description = "Already exists", Time = 3 })
        return false
    end
    local ThemeData = {
    ...
    if Ok and pcall(writefile, Path, Enc) then
        ...
        Library:Notify({ Title = "Theme Created", Description = "Created \"" .. Name .. "\"", Time = 3 })
        return true
    end
    return false
end
...
function Library:SaveTheme(Name)
    Name = tostring(Name or "")
    if not writefile or Name == "" then return false end
    EnsureFolder(Library.ThemeFolder)
    local ThemeData = {
        BackgroundColor = { Library.Scheme.BackgroundColor.R, Library.Scheme.BackgroundColor.G, Library.Scheme.BackgroundColor.B },
        MainColor = { Library.Scheme.MainColor.R, Library.Scheme.MainColor.G, Library.Scheme.MainColor.B },
    ...
    local Ok, Enc = pcall(function() return HttpService:JSONEncode(ThemeData) end)
    if Ok and pcall(writefile, Library.ThemeFolder .. "/" .. Name .. ".json", Enc) then
        Library._ThemeCache[Name] = true
        Library:Notify({ Title = "Theme Saved", Description = "Saved \"" .. Name .. "\"", Time = 3 })
        return true
    end
    return false
end
...
function Library:LoadTheme(Name)
    ...
    local function ToColor(t)
        if type(t) == "table" then return Color3.new(t[1] or 0, t[2] or 0, t[3] or 0) end
        return nil
    end
    ...
    Library:UpdateColorsUsingRegistry()
    Library:Notify({ Title = "Theme Loaded", Description = "Loaded \"" .. Name .. "\"", Time = 3 })
    return true
end
...
function Library:DeleteTheme(Name)
    ...
    Library:Notify({ Title = "Theme", Description = "Deleted \"" .. Name .. "\"", Time = 3 })
    return true
end
...
function Library:SetFont(FontFace)
    pcall(function()
        ...
        elseif typeof(FontFace) == "string" then
            local Ok, Result = pcall(function() return Font.fromEnum(Enum.Font[FontFace]) end)
            if Ok then FontFace = Result
            else
                Ok, Result = pcall(function() return Font.fromName(FontFace) end)
...
function Library:GetFontList()
    return {
        "GothamMedium", "GothamBold", "Gotham", "Code", "Ubuntu",
        "Arial", "SourceSans", "SourceSansBold", "FredokaOne",
        "Highway", "SciFi", "Roboto", "RobotoMono", "JosefinSans",
    }
end
...
function Library:ApplyPreset(Name)
    ...
    for K, V in pairs(P) do Library.Scheme[K] = V end
    Library:UpdateColorsUsingRegistry()
    Library:Notify({ Title = "Theme", Description = "Applied " .. Name, Time = 2 })
    return true
end

function Library:GetPresetNames()
    return { "BlackAndWhite", "DarkPurple", "OceanBlue", "Crimson" }
end
...
--[[
================================================================================
  OBSIDIAN UI FORK - EXTENDED API DOCUMENTATION
================================================================================
  ...
  TABS
  ----
  local Tab = Window:AddTab("Name", "icon")  -- lucide icon name

  GROUPBOXES
  ----------
  local Left  = Tab:AddLeftGroupbox("Title")
  local Right = Tab:AddRightGroupbox("Title")

  OPTIONS
  -------
  Left:AddToggle("Idx", { Text = "Label", Default = false, Callback = function(v) end })
  Left:AddSlider("Idx", { Text = "Label", Default = 50, Min = 0, Max = 100, Rounding = 0 })
  Left:AddDropdown("Idx", { Text = "Label", Values = {"A","B"}, Default = 1 })
  Left:AddInput("Idx", { Text = "Label", Default = "", Placeholder = "..." })
  Left:AddButton("Click Me", function() end)
  Left:AddLabel("Text")
  Left:AddDivider()

  ADDONS
  ------
  Toggle:AddKeyPicker("Idx", { Default = "E", Mode = "Toggle", Text = "Name" })
  Toggle:AddColorPicker("Idx", { Default = Color3.new(1,1,1), Callback = function(c) end })
  ...
  Library:GetConfigs()  --> { "name1", "name2", ... }

  THEMES
  ------
  Library:CreateTheme(name)
  Library:SaveTheme(name)
  Library:LoadTheme(name)
  Library:DeleteTheme(name)
  Library:GetThemes()
  Library:ApplyPreset("BlackAndWhite" | "DarkPurple" | "OceanBlue" | "Crimson")
  Library:GetPresetNames()
  Library:ResetTheme()
  Library:SetFont("GothamMedium")
  Library:GetFontList()

  UI CONTROL
  ----------
  Library:Toggle()              -- show/hide
  Library:Toggle(true/false)
  Library:Notify({ Title = "...", Description = "...", Time = 3 })
  Library:Unload()
  Library:SetKeybindFrameVisible(true/false)
  ...
  MOBILE
  ------
  Library.IsMobile  -- true on phone/tablet
  On mobile: smaller window + Toggle UI / Lock buttons
  On PC: full size window, no extra buttons
...
local ExtraPresets = {
    ...
    Ice = {
        BackgroundColor = Color3.fromRGB(10, 14, 18),
        MainColor = Color3.fromRGB(16, 22, 28),
        AccentColor = Color3.fromRGB(160, 220, 255),
        OutlineColor = Color3.fromRGB(35, 45, 55),
        FontColor = Color3.fromRGB(240, 250, 255),
    },
    Rose = {
        BackgroundColor = Color3.fromRGB(18, 10, 12),
        MainColor = Color3.fromRGB(26, 16, 18),
        AccentColor = Color3.fromRGB(255, 80, 120),
        OutlineColor = Color3.fromRGB(50, 30, 35),
        FontColor = Color3.fromRGB(255, 240, 245),
    },
    Lime = {
        BackgroundColor = Color3.fromRGB(10, 14, 8),
        MainColor = Color3.fromRGB(16, 22, 12),
        AccentColor = Color3.fromRGB(180, 255, 50),
        OutlineColor = Color3.fromRGB(35, 45, 28),
        FontColor = Color3.fromRGB(245, 255, 240),
    },
    Violet = {
        BackgroundColor = Color3.fromRGB(12, 8, 18),
        MainColor = Color3.fromRGB(20, 14, 28),
        AccentColor = Color3.fromRGB(180, 100, 255),
        OutlineColor = Color3.fromRGB(40, 30, 55),
        FontColor = Color3.fromRGB(245, 240, 255),
    },
}

local _OldApplyPreset = Library.ApplyPreset
function Library:ApplyPreset(Name)
    local Extra = ExtraPresets[Name]
    if Extra then
        for K, V in pairs(Extra) do
            Library.Scheme[K] = V
        end
        Library:UpdateColorsUsingRegistry()
        Library:Notify({ Title = "Theme", Description = "Applied " .. tostring(Name), Time = 2 })
        return true
    end
    return _OldApplyPreset(self, Name)
end

local _OldGetPresetNames = Library.GetPresetNames
function Library:GetPresetNames()
    local Names = _OldGetPresetNames(self)
    for Name in pairs(ExtraPresets) do
...
function Library:QuickTheme(Name)
    local Map = {
        black = "BlackAndWhite", white = "BlackAndWhite", bw = "BlackAndWhite",
        purple = "DarkPurple", blue = "OceanBlue", ocean = "OceanBlue",
        red = "Crimson", crimson = "Crimson", green = "Emerald",
        emerald = "Emerald", sunset = "Sunset", orange = "Sunset",
        midnight = "Midnight", pink = "Pink", cyber = "Cyber",
        gold = "Gold", ice = "Ice", rose = "Rose", lime = "Lime", violet = "Violet",
    }
    local Key = Map[string.lower(tostring(Name or ""))] or Name
    return Library:ApplyPreset(Key)
end
...
function Library:ListToggles()
    local Names = {}
    for Idx in pairs(Toggles) do table.insert(Names, Idx) end
    table.sort(Names)
    return Names
end
...
function Library:ListOptions()
    local Names = {}
    for Idx in pairs(Options) do table.insert(Names, Idx) end
    table.sort(Names)
    return Names
end
...
function Library:ImportConfig(Name, JsonString)
    if not writefile or not Name or not JsonString then return false end
    EnsureFolder(Library.ConfigFolder)
...
function Library:AutoSaveTheme()
    return Library:SaveTheme("autosave")
end
...
function Library:AutoSaveConfig()
    return Library:SaveConfig("autosave")
end
...
function Library:ApplyScheme(SchemeTable)
    if type(SchemeTable) ~= "table" then return false end
...
Library.Open = Library.ShowUI
Library.Close = Library.HideUI
Library.Version = Library:GetVersion()
--[[
  Additional notes / changelog
  ----------------------------
  v2.1 - Fixed executor compatibility (no type annotations on services)
  v2.1 - Safe CurrentCamera handling
  v2.1 - AutoShow forces MainFrame.Visible = true
  v2.1 - Config Create/Save/Load/Delete with notifies + cache
  v2.1 - Theme Create/Save/Load/Delete with notifies + cache
  v2.1 - Extra theme presets (Emerald, Sunset, Midnight, Pink, Cyber, Gold, Ice, Rose, Lime, Violet)
  v2.1 - QuickTheme aliases
  v2.1 - Import/Export config and theme
  v2.1 - AutoSave/AutoLoad helpers
  v2.1 - GetInfo / PrintInfo / SafeNotify
  v2.1 - ShowUI / HideUI aliases

  Supported executors: most Delta, Wave, Codex, Solara, Hydrogen, etc.
  Requires: loadstring, HttpGet for remote load
  Optional: writefile/readfile/listfiles/delfile for configs & themes
  Optional: gethui/protectgui for GUI parenting

  Mobile: TouchEnabled devices get Toggle UI + Lock buttons
  PC: RightControl toggles UI by default

  Icon source: lucide-roblox-direct (optional, fails silently if blocked)

  File size target: ~200KB for distribution stability on some hosts.
